...
> **SALVAGED FROM `claude/blocks` (2026-09-25), NOT RE-RUN HERE.** This is the
> only artefact of that branch worth keeping (the rest is the red-black-coupled
> projection `claude/jacobi-interface` was forked to replace, plus GPU exchange
> work the `map(to: c)` / index-list rewrite superseded). It is carried over so
> the branch can be deleted, and it is carried over UNVALIDATED on this branch:
> it was written against the projection of the day, and although its whole
> chain still exists here -- every config key it uses, and
> `tools/{make_channel_restart,channel_interface_validation,check_interface_decay}.py`
> -- no one has run it since. Treat the first run as a bring-up, not a gate.
> The VALIDATED 2:1-interface turbulent case is `../channel_interface`; this is
> the fast twin for iteration, with ~40x less homogeneous plane area (see the
> statistics caveat below). The one tool it names that is gone is
> `plot_field_section.py`, and only in a comment.
