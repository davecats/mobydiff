...
contains
    ...
    subroutine airfoil_setup_after_grid(this, blk, dns, g, bc, c)
        ...
        ! The control-volume budget is the ONLY force statistic, and there is
        ! no sane default box -- the case does not know where the immersed
        ! body sits. Without one, forces are DISABLED and said so, loudly,
        ! every run.
        !
        ...
        ! Re-enabling forces on one of those cases means CHOOSING a box and
        ! re-validating it: the budget is sensitive to the per-face p_inf
        ! subtraction and to borders crossing a 2:1 interface, so a box put
        ! in without a check produces numbers nobody has verified.
        if (.not. this%cv_set) then
            if (this%force_sample_interval <= 0) return
            if (c%has_terminal) print *, &
                "WARNING: [case.airfoil] has no cv_box -- runtime C_L/C_D are", &
                " DISABLED for this run. Set cv_box = c0 c1 l0 l1 around the", &
                " body to enable them (and re-validate), or set", &
                " force_sample_interval = 0 to silence this."
            this%force_sample_interval = 0
            return
        end if
        do ib = 1, 4
