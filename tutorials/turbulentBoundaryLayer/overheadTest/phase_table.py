...
def main():
    ...
    # Column label: drop the noise ("_prof", the interface-height tag) and keep
    # the tail, which carries the A/B suffix. The refined rows are the only ones
    # whose solver is implied (red-black cannot cross a 2:1 interface), so drop
    # "_jacobi" there too -- without that the truncation turns
    # refined_yp82_rect_jacobi into "rect_jacobi", which reads as the
    # SINGLE-LEVEL rect column.
    def short_label(n):
