...
### `nvfortran-Error-Unknown switch: -xCORE-AVX2` (hit and fixed 2026-09-05)
...
`-xCORE-AVX2` is an **Intel** flag, and it comes from the site environment, not
from this repo. HoreKa loads an Intel toolchain by default, which exports
`FFLAGS="-O2 -xCORE-AVX2"`. CMake seeds `CMAKE_Fortran_FLAGS` from `FFLAGS` (and
`CMAKE_C_FLAGS` from `CFLAGS`), and neither `CMakeLists.txt` nor `compile.sh`
pins those, so the environment is the only source.

**The fix is `module purge` before `module load`, plus deleting the CMake
cache.** Both halves are needed, and the second is the one that makes this
confusing to diagnose:

- **`module purge`** removes the Intel environment *coherently* — the flag
  variables, and equally the `PATH`, `LD_LIBRARY_PATH` and `CPATH` entries that
  would interfere just as easily. Both `submit.sh` (compute node) and
  `setup_and_run.sh` (login node, where `build_hdf5.sh` compiles with
  `CC=mpicc` over `nvc`) now purge first.
- **Delete the CMake cache.** CMake captures `CMAKE_Fortran_FLAGS` on the
  **first** configure and reuses the cached value afterwards *regardless of the
  environment*. So a `build_gpu/` first configured under the Intel defaults
  keeps `-xCORE-AVX2` even once the modules are right, and the build keeps
  failing after the environment is fixed. `submit.sh` now wipes a build
  directory that contains no executable (printing the cached flag line it
  discards); `build_hdf5.sh` drops its `cmbuild` for the same reason.

Recovery on an already-failed attempt — the pinned solver commit does not
change, only these scripts:

```bash
cd <the package dir>            # a clone: git pull;  rsync'd: re-rsync it
rm -rf <CODE_DIR>/build_gpu     # belt and braces; submit.sh does this itself
bash setup_and_run.sh
```

**The general rule this is an instance of:** `compile.sh` deliberately does not
pin optimisation flags — that is what lets the workstation pre-seed
`CMAKE_Fortran_FLAGS=-Mnofma` for the bit-exactness gates — so *any* inherited
`FFLAGS`/`CFLAGS` silently becomes part of the build. On a new machine, purge
before loading, and treat an unexplained flag in `CMakeCache.txt` as
environmental rather than as a repo bug. A stale cache will keep it alive long
