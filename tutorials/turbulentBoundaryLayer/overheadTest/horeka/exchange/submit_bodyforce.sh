#!/bin/bash
#SBATCH --job-name=moby_bodyforce
#SBATCH --nodes=2
...
#
# A/B for "stop doing work that does not need doing": the trip force no longer
# refreshes the whole domain every substage (block list + v component only), and
# update_ibm_mu is skipped entirely when no coefficient is non-zero on the rank.
# Readings pre-registered in PREREGISTERED_bodyforce.md, including why the gate
# runs at PRODUCTION flags here and not nofma.
#
#   REF_DIR is a worktree at cd0f279 -- the head of the register work, so this
...
STG="$RUN_DIR/bodyforce_staged"; rm -rf "$STG"; mkdir -p "$STG"
...
RES="${RESDIR:-$RUN_DIR/results_bodyforce}"; mkdir -p "$RES"
{
...
echo "=== bodyforce A/B job finished ==="
