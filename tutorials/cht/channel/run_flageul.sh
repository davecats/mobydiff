...
#
#   ./run_flageul.sh [prepare|ic|develop|settle|clean|stats]
#
...
# The case file, from the two wall-slab STLs. The prepare ini IS the run ini
# with the coefficient file swapped for the geometry and the placeholders made
# inert -- generated rather than kept as a second copy, so the two cannot
# drift apart (the run ini carries the grid, Re and the y node line, and a
# mismatch between them would not be caught: the solver's coefficient-file
...
if want prepare; then
    ...
    sed -e "s|^coeff_file = @CASE@|stl_file = wall_lo_f.stl\nstl_file = wall_hi_f.stl|" \
        ...
        -e "s|@PREFIX@|fprep|" \
        cht149_flageul.ini > .fprep.ini
    sed -i '/^\[restart\]/,$d' .fprep.ini
    time mpirun -n "${PREP_RANKS:-8}" "${PREP:-$ROOT/build_cpu/moby_prepare}" \
        .fprep.ini "$CASE" > fprep.log 2>&1 || { tail -15 fprep.log; exit 1; }
    ls -la "$CASE"
fi
