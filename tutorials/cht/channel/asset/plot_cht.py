...
FOUR PANELS, each a comparison:
  ...
  (a) theta+(y+) against KADER's correlation -- a published analytic curve, so
      ...
      beyond.
  (b) the VARIANCE for the kappa_s sweep, against their RAW conjugate curve
      (flageul_data/g1a1_fluct1.dat) and the band spanned by their two IDEAL
      brackets, which stay digitised. The headline.
...
HONESTY ABOUT THE REFERENCE. The CONJUGATE case is their own published data
(flageul_data/, fetch_flageul.py). Only the two IDEAL BRACKETS are digitised
from their figure 5 by digitize_flageul.py -- their repository publishes
conjugate cases only -- and those are good to ~0.2-0.4 in <T'^2>, because a
symbol centroid is a coarse estimator. Digitising the conjugate line itself
was biased by rms 0.15 / bias +0.13 and put its peak 4.5 % high, which is most
of the "we are low by 4 %" this campaign chased; it is no longer used. isoT is
unusable below
...
# Only the two IDEAL BRACKETS are digitised (digitize_flageul.py), from their
# symbol series -- isoQ (green x, the K -> 0 limit) and isoT (blue +, the
# K -> infinity limit). They are good to ~0.2-0.4 in <T'^2>, because a symbol
# centroid is a coarse estimator; the number is in the .dat headers, measured
# by digitising each series off BOTH panels of their fig. 5. The CONJUGATE
# reference is their raw data instead (see main()).
...
def main():
    ...
    # THE CLEAN WINDOW is the default. flageul_stats.h5 is the CONTAMINATED
    # one (t = 24..120, less than one solid diffusion time from a 24-unit
    # develop leg) and is kept only for the before/after in the README.
    ap.add_argument("--stats", default="flageul_clean.h5",
    ...
    ap.add_argument("--vel", default="Cstat_vel.h5",
    ...
    sweep = load(a.stats)
    # THE CONJUGATE REFERENCE IS THEIR RAW DATA, not the digitised figure.
    # Digitising their Fig. 5a was biased by rms 0.15 / bias +0.13 and put the
    # peak at 6.208 against the raw 5.941 -- 4.5 % high, which is most of the
    # "we are low by 4 %" that the campaign chased for weeks. The two IDEAL
    # brackets below stay digitised: their repository publishes conjugate
    # cases only, so there is no raw counterpart for those.
    raw = np.loadtxt(os.path.join(HERE, "flageul_data", "g1a1_fluct1.dat"))
    ref = np.column_stack((raw[:, 0], raw[:, 7]))
    ...
    for (nm, idx, K), c in zip(show, RAMP):
        ...
        B.annotate(f"K = {K:g}", xy=(ypc[0], v[0]), xytext=(8, 0),
                   ...
                   bbox=dict(boxstyle="round,pad=0.12", fc=SURFACE, ec="none",
                             alpha=0.85))
    B.annotate("Flageul 2015:\ndashed = their CONJUGATE case (raw data)\n"
               "dotted band = isoT to isoQ, their two\nIDEAL brackets (digitised)",
    ...
    fig.text(0.008, 0.010,
             ...
             "every curve here is a\none-to-one comparison. The conjugate reference is their PUBLISHED RAW data; only the two ideal brackets are digitised, to "
             "0.2-0.4 in <T'^2>.",
