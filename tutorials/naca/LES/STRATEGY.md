...
> **SALVAGED from `claude/jacobi-interface` (2026-09-26) when that branch was
> deleted.** This is a PLAN for work that was never started — the LES kickoff for
> the NACA case, following Atzori Sect. 2.2 for the discretisation
> (`../atzori.pdf`, salvaged with it). Nothing in `main` supersedes it, which is
> why it came over rather than being archived with the branch; nothing in `main`
> implements it either. The trip forcing it mentions as "ported from
> mobydiff.bl" IS in the solver (`[force] type = trip`, now the exact
> CaNS/SIMSON form — see CLAUDE.md), so that part of the plan is already done.
> The rest is unbuilt.
