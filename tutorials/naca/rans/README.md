...
migration, resolution and ambient studies):
docs/next_session_naca_re4e5.md, docs/next_session_skew_convection.md.

> **BoostConv is NOT in this solver.** The steady-state residual-recombination
> accelerator that the investigation trail mentions was left behind on
> `claude/jacobi-interface` (module `boostconv.f90`, `[rans] boostconv*`) when
> that branch's features were ported on 2026-09-25, on its own evidence: V1 was
> NEGATIVE on turb180 (best configuration converged 2.7x SLOWER than plain
> marching, worst 0.25x) and V2's apparent win was INVALIDATED -- the
> recombination suppressed the pressure-side ktrip strip (strip k 7.6e-3 ->
> 7.9e-6, lower-side Cf halved), so BoostConv and the forced-transition trip
> devices are incompatible as packed and the V2 coefficients were corrupted
> transition physics. The converged numbers in this README are from the PLAIN
> rerun, which is why they stand. Recover the module from `4962b10`'s successor
> history (commits c39869b, d07f673) if it is ever revisited; the fix it would
> need is to mask trip/pin cells out of the packed state.
