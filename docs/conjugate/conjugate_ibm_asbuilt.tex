...
The scheme is \textbf{second order where the interface is flat or the local
temperature gradient is interface-normal, and first order at an obliquely cut or
curved interface carrying a genuine tangential gradient.} The condition is geometric,
not a matter of resolution. It is exact --- not merely second order --- when
the interface normal is a grid direction, which covers every flat-wall case
including all of the channel validation in \S\ref{sec:validation}. At an
obliquely cut or curved interface carrying a tangential gradient the cut-face
\emph{flux} error does not converge at all; the conservative cell balance
damps that into a \emph{first-order} solution error --- measured below on both
geometries, so ``does not converge'' is a statement about the flux and never
about the scheme. Each statement is measured.

\begin{center}
\begin{tabular}{p{6.1cm}p{4.0cm}l}
\toprule
regime & what is measured & result \\
\midrule
Interface normal along a grid direction, \emph{any} cut position within the
cell; steady & 1D two-material slab, cut swept through a full cell,
$\kappa_s$ over five decades & \textbf{exact},
$\le 9.7\times10^{-15}$ \\[2mm]
The same, \emph{transient} & decaying two-material eigenmode; field error and
decay rate & \textbf{1.99, 2.00} \\[2mm]
The same, but with a \emph{pointwise} cut-cell capacity & the same eigenmode,
archived pre-C3 binary & 1.71, 1.09 --- \textbf{first order} \\[2mm]
Oblique face, gradient purely interface-normal ($r=0$) & cut-face normal flux
against the analytic conjugate field & \textbf{exact},
$\sim10^{-10}$ (geometry floor) \\[2mm]
Oblique face, tangential ratio $r>0$ --- the cut-face \emph{flux} & the same,
at $h=1/64,1/128,1/256$ & error $\propto r$, \textbf{independent of $h$}
\\[2mm]
The same, the \emph{solution} & oblique-plane conjugate BVP, solved and
refined & \textbf{first order} ($1.00$ in the interface band) \\[2mm]
Curved interface, the \emph{solution} & cylinder in a harmonic far field,
$n=64\ldots512$ & \textbf{first order} ($0.999$) \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{The two ways to be second order.}
The series resistance \eqref{eq:kface} is the \emph{exact} solution of the
one-dimensional conjugate problem, so wherever the problem is locally
one-dimensional the interface contributes no error at all and the scheme is
simply the underlying second-order discretisation. Two distinct situations
qualify, and the first is not an approximation:
\begin{itemize}
\item the interface normal is a grid direction --- every flat-wall case,
      including all of the channel validation in \S\ref{sec:validation}. The
      cut position is irrelevant: the slab gate sweeps it through a whole cell
      and the error stays at round-off;
\item the local gradient is interface-normal, $r = |\grad_t T|/|\partial_n T|
      \to 0$, which is the resolved-DNS premise the scheme was designed on.
\end{itemize}

\paragraph{Where it is not second order, and it is worse than first.}
At an obliquely cut face the baseline's implied normal flux is
$q_n + h_d s_t/(a\mathcal{R})$, and since $\mathcal{R}\sim h_d/k$ the error is
$\sim k\,s_t/a$ --- proportional to a \emph{physical} gradient and carrying no
power of $h$. Measured at $\theta=30^\circ$, $\kappa_s=10$, the rms flux error
is $3.56\times10^{-2}$, $3.77\times10^{-2}$, $3.59\times10^{-2}$ at
$h=1/64,1/128,1/256$: an observed order of $-0.01$ across a fourfold
refinement, i.e.\ flat. It scales exactly linearly with $r$
($3.56\times10^{-2}$, $3.56\times10^{-1}$, $3.56$ at $r=0.01,0.1,1$).

So at such a face the error \textbf{plateaus at $O(r)$} rather than reducing.
This is a consistency error on a codimension-one set, and how much of it
survives into the solution norm has now been measured.

\paragraph{The solution order at an oblique plane: \textbf{one}.}
On the manufactured oblique plane of \S\ref{sec:disabled} ($\theta=30^\circ$,
$\kappa_s=10$, $r=0.01$), solved and refined over $n=64,128,256$, the
interface-band $L_2$ error falls at order $\mathbf{1.00}$
($2.10\times10^{-6}$, $7.23\times10^{-7}$, $2.52\times10^{-7}$; globally
$1.53$, since the band is a shrinking fraction of the domain). So the $O(r)$
flux plateau costs one order, not convergence. For contrast, the multiplier of
\S\ref{sec:disabled} applied with the same discrete $s_t$ is $25\times$
\emph{worse} than the baseline here and still first order, while a converging
$s_t$ makes the solution exact to round-off ($1.5\times10^{-13}$): on a plane
the series resistance is already exact, so once the tangential term is right
the interface contributes nothing at all.

\paragraph{The solution order on a curved interface: \textbf{one}.}
\emph{Measured 2026-09-15, and it is a correction to what this section
previously left open.} The discrete conjugate BVP is solved on a conducting
cylinder in a harmonic far field --- exact solution known in both materials,
Dirichlet data from it, no source --- and refined. Every scheme here is linear
in $T$ with purely geometric coefficients, so its matrix is recovered exactly
by graph colouring (checked against the operator to $2\times10^{-16}$) and
solved directly: no iterative tolerance enters the error.

\begin{center}
\begin{tabular}{lcccc}
\toprule
& \multicolumn{2}{c}{as implemented (\S\ref{sec:face})}
& \multicolumn{2}{c}{with a converging $s_t$} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
$\kappa_s$ & $L_2$ & $L_\infty$ & $L_2$ & $L_\infty$ \\
\midrule
0.01 & 1.11 & 0.94 & 2.00 & 1.60 \\
0.1  & 1.10 & 0.92 & 2.04 & 1.62 \\
2    & 1.07 & 0.86 & 1.87 & 1.60 \\
10   & \textbf{1.00} & 0.91 & \textbf{1.92} & 1.78 \\
100  & 1.36 & 1.34 & 1.97 & 1.47 \\
1000 & 1.89 & 1.65 & 1.99 & 1.49 \\
\bottomrule
\end{tabular}
\end{center}

\noindent
Grids $n=32,64,128,256$ (the $\kappa_s=10$ row over $n=64\ldots512$, an
eightfold range, where the as-implemented order is $0.999$). A second geometry
--- the dipole rather than the quadrupole --- reproduces it: $1.03$--$1.09$
against $2.09$--$2.11$.

Three things follow. First, the conservative finite-volume structure
\emph{does} damp the non-converging face error, by exactly one power of $h$:
an $O(1)$ flux error on a codimension-one set becomes a first-order solution
error, not a stalled one. Second, \textbf{first order is the honest
description of the scheme at a curved conjugate interface} over the whole
physically interesting contrast range. Third, the baseline recovers second
order only as $\kappa_s\to\infty$, and for a physical reason rather than a
numerical one: $|\grad_t T|$ at the interface scales as $2/(1+\kappa_s)$, so
in the isothermal limit the term being dropped vanishes on its own. The
scheme's \emph{worst} case is therefore moderate contrast, not high --- the
opposite of what the face-flux measurements suggest.

The right-hand columns are not implemented; they are what a converging
tangential estimate delivers, measured on the same problems
(\texttt{validation/conjugate/README.md}, stages 0--1b).

\paragraph{Why that was accepted.}
Because $r\sim10^{-2}$ in a resolved thermal layer, the plateau sits near
$1\,\%$, and because the measured crossover is $r^\star\approx0.027$: below
it the corrected flux is \emph{worse} than the baseline
(\S\ref{sec:disabled}). Restoring formal second order at oblique faces is
therefore not a matter of switching the existing term on.

\paragraph{The capacity is what secures the transient order.}
Note the third row of the table: the spatial treatment above is unchanged
there, and the order still falls to first. A cut cell carrying one material's
whole heat capacity is an $O(1)$ error on an $O(h)$ band, and the decay rate
$\mu$ is a global functional of the capacity distribution, so it is visible in
every norm.
...
(2015) in thermal problem, Reynolds number, grid resolution ($\Delta y^+ =
0.49\to4.87$, $\Delta x^+ = 8.4$, $\Delta z^+ = 4.2$), solid thickness
($d^+=149$) and outer-wall boundary condition (imposed flux), compared against
\emph{their published raw data} over a clean $29\,684$ wall-unit window.
Agreement, as a percentage of each quantity's own peak:
\begin{center}
\begin{tabular}{lclc}
\toprule
$U^+$ & $0.4\,\%$ & $-\overline{u'v'}$ & $0.4\,\%$ \\
$\theta^+$ & $1.2\,\%$ & $\overline{u'T'}$ & $1.3\,\%$ \\
$\overline{u'^2}$ & $0.9\,\%$ & $\overline{v'^2}$ & $1.8\,\%$ \\
$\overline{w'^2}$ & $3.8\,\%$ & $\overline{T'^2}$ & $2.4\,\%$ \\
\bottomrule
\end{tabular}
\end{center}
The near-wall variance peak is $5.98$ against their $5.94$, $+0.7\,\%$, and
$\theta_\tau$ is $\kappa_s$-independent across the sweep to $0.003\,\%$. The
velocity comparison matters independently: it is the first against a
trustworthy reference, so the thermal agreement is not resting on a
compensating error in the flow.

\paragraph{Inside the solid.}
The scheme is also checked where the solid is the answer rather than a boundary
condition --- the variance profile \emph{through} the slab, against their
\code{fluctb} data:
\begin{center}
\begin{tabular}{lccc}
\toprule
depth $y^+$ & $-77$ & $-102$ & $-145$ (outer face) \\
\midrule
ours    & $0.0113$ & $0.0075$ & $0.0063$ \\
theirs  & $0.0113$ & $0.0083$ & $0.0073$ \\
\bottomrule
\end{tabular}
\end{center}
This one carries a methodological warning worth more than the numbers. The
first attempt read $8\times$ their value at the outer face, and the cause was
neither the scheme nor the grid: the solid diffusion time is
$d^2/\alpha_s \approx 106$ time units and the averaging window was shorter than
\emph{one}, so the statistics were dominated by the initial transient.
Splitting the variance into a within-plane (spatial) and a plane-mean
(temporal) part isolates it --- the temporal term falls by a factor $45$ when
the transient is dropped. \textbf{Start conjugate statistics only after one
solid diffusion time, and seed the solid rather than waiting for it}: with an
imposed flux at both outer faces the solid's mean level is a pure-Neumann
null-space mode, so it can be set in closed form instead of relaxed.
