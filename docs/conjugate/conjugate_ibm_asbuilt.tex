...
The scheme is \textbf{second order}, and the condition is a geometric one: the
local temperature gradient must be interface-\emph{normal} at the cut face.
That is exact when the interface normal is a grid direction, and approached in
a resolved thermal layer at any orientation. Where it fails --- an obliquely
cut face carrying a genuine tangential gradient --- the cut-face flux error
does not converge at all. Each statement below is measured.
...
This is a consistency error on a codimension-one set, so how much of it
survives into the solution norm is not settled here --- the flux is
conservative, and no global refinement study has been run for an oblique
interface with $r>0$. What is settled is the flux itself, and that it does not
converge.
