...
temperature gradient is interface-normal, and first order at a curved
interface carrying a genuine tangential gradient.} The condition is geometric,
...
obliquely cut face carrying a tangential gradient the cut-face flux error does
not converge at all; the conservative cell balance damps that into a
first-order \emph{solution} error, measured below. Each statement is measured.
...
Oblique face, tangential ratio $r>0$ & the same, at $h=1/64,1/128,1/256$ &
error $\propto r$, \textbf{independent of $h$} \\
\bottomrule
\end{tabular}
\end{center}
