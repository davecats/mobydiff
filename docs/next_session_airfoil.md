...
1. **Outlet flag into the kernels.** Derive at init a per-face flag
   "pressure-Dirichlet FACE_PHYS face" (`bc%faceBcType(VAR_P, face) ==
   DIRICHLET`), pass it as small `pdirLow(3)/pdirHigh(3)` logicals into
...
5. **apply_bc must not stomp the corrected face.** apply_bc runs inside
   ...
   `u(nx+1)` with the Neumann copy each iteration. Add a THIRD normal-
   velocity BC type `outflow` (read_bc_type value 2): apply_bc SKIPS the
   normal-component write for it (tangential ghosts keep Neumann copy;
   pressure ghost keeps its Dirichlet mirror). The outlet face is then
   owned by the projection, which is the only consistent owner. You
   cannot impose zero-gradient velocity AND Dirichlet pressure on the
   same corrected field — the pressure condition wins; zero-gradient is
   only the predictor-stage stance.
...
- `apply_defaults` composes the freestream (iii) from (i): all
  non-periodic faces get patch = `patch`; inlet-type faces (x_min, y_min,
  y_max by default) get Dirichlet velocity (U∞cosα, U∞sinα, 0) + Neumann
  p (the default); the outlet face gets normal-velocity `outflow`,
  tangential Neumann, p Dirichlet value 0. An explicit user key in
  [boundary] wins (set-if-unset, the channel apply_defaults idiom).
  z stays periodic (quasi-2D). Reject aoa so large that y_max stops being
  an inlet in spirit? No — Dirichlet freestream on y faces is the
  standard penalization far-field either way; just document that big |α|
  needs a taller domain.
...
For volume penalization, the momentum equation integrated over the whole
domain gives the body force as the penalization-term integral (boundary
fluxes + outer pressure balance the rest): **F_on_body = ∫ coef·u dV**,
component-wise on the staggered grid, and by the mu-masking identity above
...
- **Scalar inlet values** (the T4 gap, hook ready): rans.f90's mode
  tables gain SCALAR_BC_VALUE at inlet-type faces (velocity-Dirichlet
  non-wall patches) with the freestream values already computed from
  `[rans] tu` / `nut_ratio` at init (k∞ = 1.5(tu/100·U∞)², ω∞ from
  nut_ratio; γ∞ = 1; R̃e_θt,∞ = the tu correlation value T4 uses for the
  IC), and SCALAR_BC_COPY at the outlet. nut wall-ghost handling is
  untouched.
...
> `claude/jacobi-interface`. Implement phase A0: the freestream
> inflow/outflow boundary machinery. The Dirichlet-velocity inlet exists;
> the work is the Dirichlet-PRESSURE outlet in the projection
> (pressure_solver.f90): per-face pressure-Dirichlet flags into the
> kernels; phi ghost MIRRORED at the outlet via apply_scalar_bc after
> each per-iteration phi exchange; denominator counts 2·d1f·mu at the
> outlet face while the correction uses d1f against the mirrored ghost
> (keep the pair consistent and commented — SPD lives there);
> jacobi_apply's high-face branch extended to correct the outlet face
> (FACE_PHYS ∧ pressure-Dirichlet), mirroring the 2:1 owned-face
> precedent; a new normal-velocity BC type `outflow` (read_bc_type
> value 2) that apply_bc skips for the normal component only — the
> projection owns the outlet face. All new branches must be dormant
> without a Dirichlet-p face: gate (a) bit-exact (nofma, max_abs 0, CPU
> AND GPU) on min_channel / les_ibm ± refine_body / Beltrami y-slab
> (5 steps) / turb180 / wf180_y30 / lam30t. Physics gates in a new
> validation/freestream/: (b) uniform oblique flow through an empty box
> preserved exactly, mass + divergence round-off; (c) inflow/outflow
> Poiseuille vs the periodic reference, outlet-pinned pressure level,
> 10k-step drift-free; (d) Lamb–Oseen vortex exits, report reflection;
> (e) 1==4 ranks EXACT, CPU vs GPU usual level. THEN, if A0 gates clean:
> A1 (the airfoil case module: [case.airfoil] aoa/u_inf/chord,
> apply_defaults composing inlet/outlet/far-field faces + patch
> declarations, uniform-freestream init, quasi-2D nz = nb) and A2 (the
> penalization-integral force statistics F = ∫coef·u dV per component,
> device reduction + allreduce in after_step, C_L/C_D to the runtime
> file; gates: cylinder Re = 40 C_D ≈ 1.5, Re = 100 St ≈ 0.165, empty
> domain C_L = C_D = 0 exactly, ranks/GPU exact). Write the cylinder/
> airfoil STL generator in tools/ (trimesh, extruded, float32). WORKFLOW:
> one solver job at a time; sed-shortened inis for bit-exactness
> (memory: bit-exact-gates-short). Deferred, do NOT start: A3 (RANS
> scalar inlet values + SD7003 transition validation — its own session),
> the TVD upwind revisit (triggered by the measured γ-front smearing,
> not assumed), convective outlet (only if gate (d) demands it),
