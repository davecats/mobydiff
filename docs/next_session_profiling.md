...
>
> The step-1 measurement it asked for is kept below under "Step 1 — MEASURED":
> the projection is 78–83 % of the step and its halo exchange costs as much as
> its Jacobi kernels (13.9 vs 10.6 ms/step on the doc's case). Both halves of
> that reading have since been overtaken — see the `overheadTest/results_*.md`
> series, where the exchange's cost turned out to be kernel LAUNCHES rather than
> transport.
